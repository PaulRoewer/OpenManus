...
from app.agent.manus import Manus  # Use the provided OpenManus agent
...
async def handle_task(index):
    ...
    repo_dir = os.path.join(config.workspace_root, f"repo_{index}")  # Use unique repo directory per task
    ...
    try:
        ...
        print(f"Launching agent (Manus)...")
        agent = Manus()
        await agent.run(full_prompt)
